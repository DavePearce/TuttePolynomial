...
//  memset(hashtable,  ZERO, HASHTABLESIZE * sizeof(/*hashtable[0]*/unsigned long int));
...
/*
  ...
  hashtable[hashindex] = (unsigned long int) p;
*/
...
NEWHash[hashindex].noBytes = noByte;
#endif
  return;
} /* end  */
...
/*
while (cp = (int *) hashtable[hashindex]) 
*/
  {
...
Procedures Called:
sizeof(int)
malloc(INTS_PER_BLOCK * sizeof(int))
(int *) &(block[currentblock][intsleft])
printf ("ERROR - otreea[ZERO] is %d\n", otreea[ZERO]);
*/ 
...
if(L==0 && llZeroCtr>7){printT(tutteMat); L=1;}
...
if(L==0 && intZeroCtr>7){printT(tutteMat); L=1;}
...
printf("currentblock = %2d  no of block used for hashing GraphsInHashTable = %d\n intsleft = %d \n",
                   currentblock,GraphsInHashTable,intsleft);
     intsleft = INTS_PER_BLOCK;
  }
     
intsleft -= intcount;
cp = (unsigned int *) &(block[currentblock][intsleft]);
...
/*printf("currentblock = %6d\n",currentblock);*/
  ...
  /*if (resetcount)
    printf ("HASHTABLE RESETS: %3d\n", resetcount);*/
 /* if (insertcoll)
    printf ("INSERT COLLISIONS: %3d\n", insertcoll);*/

 /* if (otreezerocount)
    printf ("otree zero count = %d\n", otreezerocount);*/
...
/*printf("currentblock=%6d\n",currentblock);*/
 /* if (insertcoll)
  ...
  printf("threshold   graphs   matches   ratio\n");
  for (i=126; i;--i)
    {
      if (graphcount[i])
	{
	  printf("%6d %9d %9d   %7.2f\n", i, graphcount[i], 
matchcount[i],
		 ((double) (matchcount[i]))/graphcount[i]);
	  msum += matchcount[i];
	  gsum += graphcount[i];
	}
    }
  printf("total  %9d %9d", gsum, msum);
  if (gsum)
    printf("   %7.2f",((double) msum)/gsum);
  printf("\n");
