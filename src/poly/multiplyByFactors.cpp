...
if( runup == ZERO )
{
for( i = ZERO; i <= lastnonzeroRow + getYPower(g)/*TUTTEROWS*/; ++i )
for( j = ZERO; j <= lastnonzeroColumn + gminusone + gminustwo /*TUTTECOLS*/; ++j )
  {
/* incorporate lastnonzerorowincolumn info */
...
for( i = ZERO; i <= lastnonzeroRowinColumn[j] + runup + getYPower(g)/* TUTTEROWS*/; ++i )
  {
     if( tutteMat1[i][j] != ZERO )
       tutteMat[i][j] += tutteMat1[i][j];
     if( tutteMat1X[i][j] != ZERO )
